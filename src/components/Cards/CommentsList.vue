	...
	<v-card class="elevation-10" id="comments">
		...
		<v-list>
          ...
          <router-link :to="'/user/' + c.user._id">
  					<v-list-tile-avatar>
  						<v-img position="top left" :src="c.user.img" alt="avatar" size="100" />
  					</v-list-tile-avatar>
          </router-link>
        ...
        <template v-for="r in c.reply">
          ...
          <v-list-tile avatar class="pl-5">
            ...
            <router-link :to="'/user/' + r.user._id">
              <v-list-tile-avatar>
                <v-img position="top left" :src="r.user.img" alt="avatar" size="100" />
              </v-list-tile-avatar>
            </router-link>
	...
	export default {
		...
		methods: {
          ...
          try {
            if (user) {
              ...
              const responce = await CardsService.addComment({ id: this.id, user_id: user, text: this.text })
      ...
      async fetchComments (step) {
        try {
          ...
          const response = await CardsService.fetchComments({ id: this.id, pn: this.pn, pz: this.pz })
