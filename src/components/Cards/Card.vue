<template>
  <v-card class="mb-2 elevation-10">
    ...
    <v-card-text>
      <v-layout row wrap>
        ...
        <div v-if="position">
          ...
          <strong>{{
            distan(position.lat, position.lng, 48.9344363, 24.6999686)
          }}</strong>
        </div>
      </v-layout>
    </v-card-text>
    ...
    <v-card-actions>
      ...
      <v-btn flat :to="'/card/' + card.id + '#comments'">
...
import { distance } from "@/mixins/index";
