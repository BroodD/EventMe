...
export default {
	...
	actions: {
        ...
        const responce = await UsersService.findByLoginMin({ login: login })
          ...
          const responce = await UsersService.addNewUser({ login, name, bio: ''})
          const id       = responce.data.id

					const userFb = await firebase.auth().createUserWithEmailAndPassword(email, password)
          ...
          // some stuff with img
					...
					await firebase.storage().ref(`users/${key}.${imageExt}`).put(img)
        ...
        if(user)
          commit('setUser', new User(user._id, user.login, user.name, user.img, user.bio, email))
				commit('setLoading', false)
			} catch (error) {
				commit('setLoading', false)
				commit('setError', error.message)
    ...
    //         .then()
        ...
        console.log('changeInfo login, name, image, bio, email', login, name, image, bio, email );
				...
				if (typeof image == 'object') {
					...
					await firebase.storage().ref(`users/${user.id}.${imageExt}`).put(image)
						.then(snapshot => snapshot.ref.getDownloadURL())
						.then(url => imageSrc = url)
				} else {
					imageSrc = image
        ...
        const responce = await UsersService.findByLoginMin({ login: login })
            ...
            id: user._id,
					...
					commit('setUser', new User(user._id, login, name, imageSrc, bio, email))
        } else {
    ...
    async userCards ({ commit, getters }, { id }) {
      ...
      try {
        var resultCards = []
        const responce = await UsersService.userCards({ id, user_id: getters.userId })
