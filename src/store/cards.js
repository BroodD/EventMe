...
export default {
  ...
  actions: {
      ...
      try {
        ...
        var imageSrc = []
				...
				for (let i = 0; i < files.length; i++) {
					...
					let fileData = await firebase.storage().ref(path).put(files[i])
				...
				if (uid === ownerId) {
					...
					if(images) {
						images.forEach(s => {
							...
							firebase.storage().ref("cards/" + name).delete()
						})
					}
		...
		async editCard({ commit, getters }, { id, update, orderImg, files }) {
            ...
            if(response.data.card) {
  						...
  						for (let i = 0; i < files.length; i++) {
  							...
  							await firebase.storage().ref(path).put(files[i])
  								.then(snapshot => snapshot.ref.getDownloadURL())
  								.then(url => imageSrc.push(url))
  						}
    ...
    async fetchCards ({commit, getters}, skip) {
      ...
      try {
        const response    = await CardsService.fetchCards({ user_id: getters.userId, position: getters.get('position') })
		...
		async fetchSingle({ commit, getters }, { id }) {
			...
			try {
        const response    = await CardsService.findById({ id: id, user_id: getters.userId })
		...
		async toggleLike({ commit, getters }, { id, has }) {
			...
			try {
				if( user_id ) {
          // type true -- like
          await CardsService.updateLikeVisitCard({ id, has, user_id, type: true })
      ...
      var user_id = getters.userId
          ...
          await CardsService.updateLikeVisitCard({ id, has, user_id, type: false })
